...
class Page{
    ...
    public function show() {
        ...
        $up_page = $up_row > 0 ? '<li class="paginate_button previous"><a href="javascript:void(0)" onclick=' . $this->callback . '(\'' . $this->url($up_row) . '\')>' . $this->config['prev'] . '</a></li>' : '';

        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = ($down_row <= $this->totalPages) ? '<li class="paginate_button next"><a href="javascript:void(0)" onclick=' . $this->callback . '(\'' . $this->url($down_row) . '\')>' . $this->config['next'] . '</a></li>' : '';

        //第一页
        $the_first = '';
        if($this->totalPages > $this->rollPage && ($this->nowPage - $now_cool_page) >= 1){
            $the_first = '<li class="paginate_button previous"><a href="javascript:void(0)" onclick=' . $this->callback . '(\'' . $this->url(1) . '\')>' . $this->config['first'] . '</a></li>';
        }

        //最后一页
        $the_end = '';
        if($this->totalPages > $this->rollPage && ($this->nowPage + $now_cool_page) < $this->totalPages){
            $the_end = '<li class="paginate_button end"><a href="javascript:void(0)" onclick=' . $this->callback . '(\'' . $this->url($this->totalPages) . '\')>' . $this->config['last'] . '</a></li>';
        }
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page <= $this->totalPages){
                    $link_page .= '<li class="paginate_button"><a href="javascript:void(0)" onclick=' . $this->callback . '("' . $this->url($page) . '")>' . $page . '</a></li>';
                }else{
                    break;
                }
            }else{
